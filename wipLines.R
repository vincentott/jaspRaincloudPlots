...
ggplot(data, aes(x = Measurement, y = Value, color = Condition)) +
  ...
  stat_summary(fun.y = mean, geom = "line", size = 1, aes(group = Condition)) +  # Add line connecting means
...
p + geom_errorbar(aes(ymin = 1, ymax = upper), width = 0.2)
